/**
 * RED PHASE — audit sink bridge for the hermes extension.
 *
 * The hermes extension writes one JSONL line per decision to the audit sink
 ...
 *
 * Same JSONL bridge semantics as tests/pi/audit.test.ts — the audit contract
 * is agent-agnostic (decision-output.v1 is the shared schema).
 *
 * Fails until src/hermes/audit.ts exists.
 */
import { describe, expect, it } from 'bun:test';
import { writeAuditEntry } from '../../src/hermes/audit';

describe('hermes-opa-net audit bridge', () => {
  it('writes a JSONL line with required fields when sessionId present', async () => {
    const written: unknown[] = [];
    await writeAuditEntry({
      sessionId: '/tmp/fake-hermes-session.jsonl',
      decision: {
        decision: 'deny',
        source: 'opa',
        reasons: [
