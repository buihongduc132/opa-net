/**
 * RED PHASE — audit sink bridge for the zcode extension.
 *
 * The zcode extension writes one JSONL line per decision to the audit sink
 ...
 *
 * Same JSONL bridge semantics as tests/hermes/audit.test.ts — the audit contract
 * is agent-agnostic (decision-output.v1 is the shared schema).
 *
 * Fails until src/zcode/audit.ts exists.
 */
import { describe, expect, it } from 'bun:test';
import { writeAuditEntry } from '../../src/zcode/audit';

describe('zcode-opa-net audit bridge', () => {
  it('writes a JSONL line with required fields when sessionId present', async () => {
    const written: unknown[] = [];
    await writeAuditEntry({
      sessionId: '/tmp/fake-zcode-session.jsonl',
      decision: {
        decision: 'deny',
        source: 'opa',
        reasons: [
    ...
    expect(entry.evaluated_at).toBe('2026-07-22T04:00:00.000Z');
  });
