#!/usr/bin/env bun
/**
 * hermes-opa-net — OPA-backed bash command guard with structured --json output.
 *
 * Usage:
 *   hermes-opa-net eval "<command>"              # claude-code mode (default)
 *   hermes-opa-net eval "<command>" --json        # full schema on stdout always
 *   hermes-opa-net eval "<command>" --unlock <key>  # present unlock key
 *   echo "<command>" | hermes-opa-net eval        # read from stdin
 *   hermes-opa-net unlock-key <rule_id>           # mint long-lived key
 *   hermes-opa-net unlock-key <rule_id> --ttl 3600 # mint TTL key
 *   hermes-opa-net unlock-key --list              # list unlockable rule_ids
 *
...
function printHelp() {
  console.error(`hermes-opa-net — OPA-backed bash command guard

Usage:
  hermes-opa-net eval "<command>" [--json] [--policy <path>] [--unlock <key> ...]
  echo "<command>" | hermes-opa-net eval
  hermes-opa-net unlock-key <rule_id> [--ttl <sec>]
